...
public class StudentDao extends GenericDao<Student> {
    ...
    public Optional<Student> findByIndexNumber(final int indexNumber) {
        //TODO - implement
        try {
            Student student = currentSession().createQuery("SELECT s FROM Student s WHERE s.indexNumber = :index_number", Student.class)
                    .setParameter("index_number", indexNumber).getSingleResult();
